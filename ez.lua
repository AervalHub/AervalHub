...
local sNames = {"textUpdateToggle", "textUpdateDelay", "serverHopToggle", "serverHopDelay", "hexBox", "goalBox","danceChoice", "thanksMessage", "signToggle", "customBoothText", "signUpdateToggle", "signText", "signHexBox", "autoThanks", "autoBeg", "begMessage", "begDelay", "fpsLimit", "render", "thanksDelay", "vcServer",'donationJump'}
...
local serverHopToggle = Booth:AddSwitch("Auto Server Hop", function(bool)
    if settingsLock then
        return
    end
...
local serverHopDelay = Booth:AddSlider("Server Hop Delay (M)", function(x)
    if settingsLock then
       return 
    end
...
Booth:AddLabel("Timer resets after donation")
if vcEnabled then
    local vcToggle = Booth:AddSwitch("Voice Chat Servers", function(bool)
        if settingsLock then
            return
        end
...
Booth:AddButton("Server Hop", function()
    serverHop()
end)
--Other tab
...
update()
if game:GetService("CoreGui").imgui.Windows.Window.Title.Text == "Speedware - Pls Donate" then
    game:GetService("CoreGui").imgui.Windows.Window.Title.Text = "Speedware - Pls donate"
end
